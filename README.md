...
> [!IMPORTANT]
> Lingua Study 1.4.0 仅支持桌面版 Obsidian。移动端仍处于测试阶段，不包含在本次正式发布范围内。当前主要面向“英文视频字幕 → 简体中文学习”的场景。公开 YouTube 字幕导入使用非官方公开接口，因为 [YouTube 官方字幕下载 API](https://developers.google.com/youtube/v3/docs/captions/download) 只能下载用户有权编辑的视频字幕。YouTube 与哔哩哔哩的公开接口可能随时变化。本插件不会绕过登录、地区、嵌入、会员或反机器人限制。

## 近期更新（1.3.0–1.4.0）

- **可选界面外观**：默认保留经典外观，也可在设置中切换为 Lingua Paper；两种外观的功能保持一致
- **播放器自由缩放**：桌面端可从播放器四个角拖动调整大小，控制器同步缩放并记住宽度
- **版本更新提醒**：打开设置页时检查最新官方 Release，仅在发现新版时提供 Obsidian 社区插件页面入口
- **逐句听写**：反复播放当前句，提交后进行逐词对照与评分
- **影子跟读**：跟随原声录音，支持暂停、继续、波形显示和录音回放
- **生词学习闭环**：双击查词、加入生词本、保留视频原句并安排每日复习
...
- 打开 Lingua Study 设置页时检查 GitHub 最新正式 Release，只在有新版时提供官方社区页面的更新入口
...
当前版本暂不支持自定义请求头、Anthropic 兼容接口、Ollama 和其他翻译目标语言。可以选择启用整篇翻译；插件会按顺序逐句请求，并自动跳过已有结果的句子。
...
2. 点击 **加入生词本**，保存单词、当前视频原句和时间戳；也可以补充个人备注。
...
电脑端还可以在同一设置页导入自定义 CSV、TSV 或 JSON 词典，并可分别下载三种模板。CSV 和 TSV 使用 `word`、`phonetic`、`definition`、`translation`、`pos`、`tags` 列；JSON 使用相同字段，顶层可以是词条数组，也可以是包含 `entries` 数组的对象，`tags` 可填写字符串或字符串数组。三种格式都支持对应的中文字段名；`word` 必填，英文释义和中文释义至少填写一项。自定义词典只在本机处理，最多 100 MB、1,000,000 个词条；同一文件中的重复单词以后出现的记录为准。查词顺序为自定义词典、ECDICT 完整版、内置精简版，删除自定义词典不会影响 ECDICT 或生词本。
