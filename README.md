...
如果所在网络无法连接 Hugging Face，可在“设置 → Lingua Study → 文稿导入与对齐 → 备用模型下载地址”填写可信的 HTTPS 模型站点根地址。该站点必须直接提供与官方相同路径、相同固定版本的模型文件；留空仍使用 Hugging Face。切换来源可能需要重新下载模型，音视频始终留在本机。某些镜像会重定向回 Hugging Face，无法解决原站不可访问的问题。
