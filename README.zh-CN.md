...
双击一个英文单词会执行完全离线的词典查询，不会调用 AI 服务。压缩后的 ECDICT 考试与词频子集已经内置于 `main.js`。用户还可以在 **设置 → Lingua Study → 学习与词典** 中下载完整 ECDICT；文件会经过校验并在本地建立索引，保存在操作系统缓存目录，而不是 Obsidian 仓库。发音按钮使用操作系统已经安装的英文语音。
